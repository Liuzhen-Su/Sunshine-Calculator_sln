...
{
    public partial class Timelength : Form
    {
            ...
            if (Weidu == -1000)
            {
                foreach (var v in groupBoxCity.Controls)
                {
                    ...
                    if (r.Checked == true)
                    {
                        ...
                        calculate(Month, Day,Weidu);
                    }
                }
                if (Weidu == -1000)
                {
                    textBoxInfo.Text = "请选择城市或正确输入纬度！";
                    return;
                }
            }
            else calculate(Month, Day, Weidu);
        }
        ...
        private void Timelength_Load(object sender, EventArgs e)
        {
        ...
        private void calculate(double month, double day, double weidu)
        {
            //时差E表格
            int[,] a ={
                { -3,-5,-7,-8,-10,-11,-12,-13},
            ...
            if (weidu > 90 || weidu < -90 || month > 12 || month < 0 || day > 31 || day < 0)
            {
                textBoxInfo.Text = "请正确输入时间和纬度！";
                return;
            }
